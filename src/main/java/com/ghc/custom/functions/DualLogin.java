...
public class DualLogin extends Function {
	 ...
	 * Constructor
	 */
	public DualLogin(Function host, Function user, Function pass, Function runcommand, Function pass1,
			Function command, Function stdout, Function stderr) {
		m_fHost = host;
		m_fUsername = user;
		m_fPassword = pass;
		m_fPassword1 = pass1;
		m_f_runCommand = runcommand; 
		m_fCommand = command;
		m_fStdOut = stdout;
		m_fStdErr = stderr;
	}

	protected DualLogin(Function host, Function user, Function pass, Function runcommand, Function pass1,
			Function command) {
		m_fHost = host;
		m_fUsername = user;
		m_fPassword = pass;
	...
	public String getSyntax() {
		return FUNCTION_NAME
				+ "(Host, Username, Password, pbrun, pwd, Command[, StdOutTag, StdErrTag])";
	}
	...
	public Function create(int size, Vector params) {
		if (params.size() > 5) {
			return new DualLogin((Function) params.get(0),
	...
	public Object evaluate(Object data) {
		...
		String runCmd = m_f_runCommand.evaluateAsString(data);
		String pwd1 = m_fPassword1.evaluateAsString(data);
		...
		try {
			...
			PasswordAuthenticationClient passwd = new PasswordAuthenticationClient();
			...
			if (result == AuthenticationProtocolState.COMPLETE) {
				...
				session.executeCommand(runCmd);
				passwd.setPassword(pwd1);
